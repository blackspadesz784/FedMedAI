...
async function loadVizGallery() {
  ...
  grid.innerHTML = items
    .map((v) => {
      const imgHtml = v.url
        ? `<img src="${getFullUrl(v.url)}" style="width:100%; height:180px; object-fit:cover; border-radius:var(--radius-md); margin-bottom:12px;" alt="${v.title}" />`
        : `<div class="viz-placeholder">Generated by training pipeline<br/>(saved to backend/docs_figures)</div>`;
      return `
